...
<script type="text/ecmascript-6">
  import CubePicker from '@c/address/components/bscroll/picker.vue'
  ...
  export default {
    ...
    methods: {
      show() {
        this.oldVal = Object.assign({},this.pickerData)
        let arr = []
        for(let i in this.oldVal) {
          arr.push(this.oldVal[i])
        }
        this.oldVal = arr
      ...
      updatePickerData(fromColumn = 0) {
        ...
        while (data) {
          ...
          data = data.length ? data[this.pickerSelectedIndex[i]].childs : null
          i++
        }
